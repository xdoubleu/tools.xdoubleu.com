...
- **trains** — SNCB/NMBS journey planner that stays useful when a trip is delayed mid-journey: a route overview between two stations and a live journey view that re-plans an alternative when a delay breaks it (issue #1388). Routes are computed in-house from the Belgian Mobility Company's open GTFS timetable, refreshed daily. In progress — as of issue #1391 the timetable ingest and the journey-search RPC exist; the `/trains` page itself is a later slice.
