...
| [adr-0024](adr-0024-posthog-product-analytics.md) | PostHog Cloud (EU) product analytics + session replay, on by default with no consent gate; `window.__ENV__` runtime injection over build-time `NEXT_PUBLIC_*`; new `auth.v1.GetCurrentUserResponse.user_id` field for `posthog.identify()` | #1637, #1638 |
...
| [spec-routine-nightly-maintenance-sweep](spec-routine-nightly-maintenance-sweep.md) | The nightly claude.ai routine's exact prompt text and required connectors (`tools-apps` MCP server + GitHub) for manual creation in the routines UI — the trigger-creation API silently drops connectors, per #1438; the prompt now also drives `monitoring-sweep`'s step 2 Sentry-resolution backstop, #1786 | #1446, #1438, #1441, #1786 |
| [spec-routine-posthog-ux-discovery](spec-routine-posthog-ux-discovery.md) | The weekly claude.ai routine's exact prompt text (runs `posthog-ux-discovery` unattended), its three required connectors (`tools-apps` + GitHub + PostHog's MCP server), and the genuine-finding thresholds that avoid weekly board spam | #1639, #1637, #1638, #1438 |
