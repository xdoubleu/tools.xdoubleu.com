...
Before making changes, pull the latest `main` and branch off it — don't assume a currently checked-out branch/worktree is still current; it may already be merged.
...
One-time setup per clone: `git config core.hooksPath .githooks` installs a `pre-push` hook that blocks direct pushes to `main` (see "never push to `main` directly" below) — nothing else enforces this today.
...
`.github/workflows/main.yml` orchestrates reusable workflows (`proto-check`, `build-api`, `build-web`, `build-gateway`, `api-lint`, `api-test`, `web-lint`, `web-test`, `gateway-test`) gated by a `changes` path filter. `gateway-test` runs on a `macos-14` runner (needs to compile cgo/AppKit).
