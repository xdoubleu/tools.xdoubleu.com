...
`web` also ships PostHog Cloud (EU) product analytics + session replay (issue #1638) — a materially different telemetry path from the aggregate-only Web Vitals beacon that feeds `web_vitals_seconds`: per-user autocapture and full session recording, on by default for every family member with a one-line passive disclosure in `/settings`, no consent gate → [`docs/adr-0024-posthog-product-analytics.md`](docs/adr-0024-posthog-product-analytics.md). A weekly scheduled routine (epic #1637's second sub-issue) mines it for friction signals and files proposed UX-improvement issues — it never applies a UI change or opens a PR itself.
