...
//   - translations.txt (issues #1450, #1459) exercises all three ways a row
//     reaches a stop: by full record_id (Bruxelles-Midi), by record_id
//     without the gateway's "gs:nmbssncb:" prefix (Antwerpen-Centraal), and
//     by field_value, which matches a station and its platform at once
//     (Gent-Sint-Pieters). It also carries a non-stops-table row, a
//     non-stop_name field row, a row identifying neither a record nor a
//     value, a blank translation row, an unrecognized-language row — every
//     one of which must be skipped — and one well-formed row naming a stop
//     this feed does not contain, which must be counted as unmatched
func SampleFeedFiles() map[string]string {
	return map[string]string{
		...
		"translations.txt": "field_name,field_value,language,record_id," +
			"table_name,translation\n" +
			// matched by full record_id
			"stop_name,,nl,gs:nmbssncb:S8814001,stops,Brussel-Zuid\n" +
			"stop_name,,en,gs:nmbssncb:S8814001,stops,Brussels-South\n" +
			// matched by record_id with the gateway prefix stripped
			"stop_name,,fr,8821006,stops,Anvers-Central\n" +
			// matched by field_value: hits the station and its platform
			"stop_name,Gent-Sint-Pieters,fr,,stops,Gand-Saint-Pierre\n" +
			"stop_name,Gent-Sint-Pieters,en,,stops,Ghent-Sint-Pieters\n" +
			...
			// identifies neither a record nor a value: skipped.
			"stop_name,,nl,,stops,Nothing-To-Match\n" +
			...
			// well-formed, but names a stop this feed does not contain: it
			// is read and counted, then reported as unmatched.
			"stop_name,,nl,gs:nmbssncb:S9999999,stops,Nergenshuizen\n",
	}
}
