...
// to "currently-reading" when it was still "to-read", "dropped", or "owned" (any
// other status, e.g. already reading or read, is left unchanged). current_page is
// untouched — Kobo only reports a percent. Single atomic statement, no
// read-modify-write race, no other fields clobbered.
func (repo *BooksRepository) UpdateLibraryProgress(
	...
	query := `
		...
		SET progress_percent = $3,
		    progress_mode = $4,
		    status = CASE WHEN status IN ($6, $7, $8) THEN $5 ELSE status END,
		    updated_at = now()
