...
-- re-pointed at books.set_updated_at() below so the backlog schema can be
-- dropped.
...
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE backlog.books SET SCHEMA books;
        ALTER TABLE backlog.user_books SET SCHEMA books;
        ALTER TABLE backlog.book_files SET SCHEMA books;
        ALTER TABLE backlog.book_reading_state SET SCHEMA books;
        ALTER TABLE backlog.kobo_devices SET SCHEMA books;
    ELSE
        ...
        CREATE UNIQUE INDEX books_isbn13_idx ON books.books (
            isbn13
        ) WHERE isbn13 IS NOT NULL;

        CREATE TABLE books.user_books (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            user_id TEXT NOT NULL,
            book_id UUID NOT NULL REFERENCES books.books (
                id
            ) ON DELETE CASCADE,
        ...
        CREATE INDEX idx_user_books_user_id ON books.user_books (user_id);
        CREATE INDEX idx_user_books_status ON books.user_books (status);

        CREATE TABLE books.book_files (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            book_id UUID NOT NULL REFERENCES books.books (
                id
            ) ON DELETE CASCADE,
            ...
            source_file_id UUID REFERENCES books.book_files (
                id
            ) ON DELETE SET NULL,
            created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
            updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
        ...
        CREATE INDEX idx_book_files_book_id ON books.book_files (book_id);
        CREATE INDEX idx_book_files_user_id ON books.book_files (user_id);
        CREATE INDEX idx_book_files_book_format ON books.book_files (
            book_id, format
        );
        CREATE INDEX idx_book_files_checksum ON books.book_files (checksum);
        CREATE INDEX idx_book_files_storage_key ON books.book_files (
            storage_key
        );

        CREATE TABLE books.book_reading_state (
            user_id TEXT NOT NULL,
            book_id UUID NOT NULL REFERENCES books.books (
                id
            ) ON DELETE CASCADE,
        ...
        CREATE TABLE books.kobo_devices (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            user_id TEXT NOT NULL,
        ...
        CREATE INDEX idx_kobo_devices_user_id ON books.kobo_devices (user_id);
    END IF;
END $$;
-- +goose StatementEnd
...
DROP TRIGGER IF EXISTS trg_user_books_updated_at ON books.user_books;
CREATE TRIGGER trg_user_books_updated_at
BEFORE UPDATE ON books.user_books
FOR EACH ROW EXECUTE FUNCTION books.set_updated_at();

DROP TRIGGER IF EXISTS trg_book_files_updated_at ON books.book_files;
CREATE TRIGGER trg_book_files_updated_at
BEFORE UPDATE ON books.book_files
FOR EACH ROW EXECUTE FUNCTION books.set_updated_at();

DROP TRIGGER IF EXISTS trg_reading_state_upd_at ON books.book_reading_state;
CREATE TRIGGER trg_reading_state_upd_at
BEFORE UPDATE ON books.book_reading_state
FOR EACH ROW EXECUTE FUNCTION books.set_updated_at();
-- +goose StatementEnd
