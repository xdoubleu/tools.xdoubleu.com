...
// Dropped has no dedicated LibraryResponse field, so groupByStatus always
// surfaces it as an empty shelf regardless of registry state — even for a
// library that never touched it and has no books.shelves row for it
// (issue #593: a shelf already emptied before the shelves registry existed
// has no user_books row left to register it from, so depending on the
// registry alone can never recover it).
