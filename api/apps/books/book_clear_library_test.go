...
func cleanupClearLibraryUser(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM books.user_books WHERE user_id = $1`, clearLibraryUser)
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM books.book_files WHERE user_id = $1`, clearLibraryUser)
		_, _ = testDB.Exec(
			context.Background(),
...
func TestClearLibrary_Service_DeletesUserBooks(t *testing.T) {
	...
	err = testDB.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM books.user_books WHERE user_id = $1`, clearLibraryUser,
	).Scan(&count)
	require.NoError(t, err)
...
func TestClearLibrary_Service_DoesNotTouchSharedCatalog(t *testing.T) {
	...
	var before int
	err := testDB.QueryRow(
		context.Background(), `SELECT COUNT(*) FROM books.books`,
	).Scan(&before)
	require.NoError(t, err)
...
func TestConnectClearLibrary_OK(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM books.user_books WHERE user_id = $1`, userID)
	})

	client := newBooksTestClient(t)
	req := connect.NewRequest(&booksv1.ClearLibraryRequest{})
