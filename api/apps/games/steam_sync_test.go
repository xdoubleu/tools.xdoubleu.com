...
func newSyncTestApp(
	...
	app := games.NewInner(
		...
		testCfg,
		testDB,
		func(_ string) steam.Client { return client },
	)
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(ctx,
			`DELETE FROM games.steam_achievements WHERE user_id = $1`, userID)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM games.steam_games WHERE user_id = $1`, userID)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM games.progress WHERE user_id = $1`, userID)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM games.user_integrations WHERE user_id = $1`, userID)
	})
...
func TestSteamWithTx_CommitAndRollback(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(ctx,
			`DELETE FROM games.steam_games WHERE user_id = $1`, user)
	})
...
func TestSyncGame_UnconfiguredCreds(t *testing.T) {
	...
	app := games.NewInner(
		...
		testCfg,
		testDB,
		func(_ string) steam.Client { return nil },
	)
