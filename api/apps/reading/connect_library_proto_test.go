...
// "/reading/api/cover/{bookId}" (see cover_routes.go), so the proxy URL built
...
func TestCoverProxyURL_UsesBooksPrefix(t *testing.T) {
	...
	assert.Equal(t, "http://api.test/reading/api/cover/"+bookID.String(), got)
}
...
func TestProtoBook_CoverURLUsesBooksPrefix(t *testing.T) {
	...
	assert.Equal(t, "http://api.test/reading/api/cover/"+book.ID.String(), pb.CoverUrl)
}
