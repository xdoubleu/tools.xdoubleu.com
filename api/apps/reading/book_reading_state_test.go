...
func TestConnectUpdateReadingProgress_Valid(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.UpdateReadingProgressRequest{
...
func TestConnectGetReadingState_Found(t *testing.T) {
	...
	// Set state first
	setReq := connect.NewRequest(&readingv1.UpdateReadingProgressRequest{
		BookId:  book.ID.String(),
		Source:  models.ReadingSourceManual,
		Percent: 33,
	})
	...
	// Get it back
	getReq := connect.NewRequest(&readingv1.GetReadingStateRequest{
		BookId: book.ID.String(),
	})
...
func TestConnectGetReadingState_NotFound(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.GetReadingStateRequest{
		BookId: book.ID.String(),
	})
	req.Header().Set("Cookie", accessToken.String())
