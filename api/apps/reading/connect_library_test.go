...
type booksTestClient struct {
	readingv1connect.LibraryServiceClient
...
func newBooksClientFor(url string, opts ...connect.ClientOption) booksTestClient {
	return booksTestClient{
		LibraryServiceClient: readingv1connect.NewLibraryServiceClient(
			http.DefaultClient, url, opts...,
		),
		BookFilesServiceClient: readingv1connect.NewBookFilesServiceClient(
			http.DefaultClient, url, opts...,
		),
		KoboServiceClient: readingv1connect.NewKoboServiceClient(
			http.DefaultClient, url, opts...,
		),
...
func TestConnectGetBooksProgress_WithDateRange(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.GetBooksProgressRequest{
...
func TestConnectSearchLibrary_Empty(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.SearchLibraryRequest{
		Query: "",
	})
	req.Header().Set("Cookie", accessToken.String())
...
func TestConnectSearchExternal_Empty(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.SearchExternalRequest{
		Query: "",
	})
	req.Header().Set("Cookie", accessToken.String())
...
func TestConnectGetExternalBook_Found(t *testing.T) {
	...
	// protoExternalBook).
	req := connect.NewRequest(&readingv1.GetExternalBookRequest{
		Provider:   "hardcover",
...
func TestConnectUpdateProgress_Pages(t *testing.T) {
	...
	req := connect.NewRequest(
		&readingv1.UpdateProgressRequest{
			BookId:       book.BookID.String(),
