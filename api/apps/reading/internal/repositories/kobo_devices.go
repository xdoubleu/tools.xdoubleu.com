...
func (r *KoboDevicesRepository) CreateKoboDevice(
	...
	err := r.db.QueryRow(ctx, `
		INSERT INTO reading.kobo_devices (user_id, name, serial, token_hash)
...
func (r *KoboDevicesRepository) DeleteKoboDevice(
	...
	tag, err := r.db.Exec(ctx, `
		DELETE FROM reading.kobo_devices WHERE id = $1 AND user_id = $2
	`, deviceID, userID)
	if err != nil {
		return err
	}
...
func (r *KoboDevicesRepository) GetLastSeenAt(
	...
	err := r.db.QueryRow(ctx, `
		SELECT max(last_seen_at) FROM reading.kobo_devices WHERE user_id = $1
