...
func (repo *BooksRepository) UpsertBook(
	...
	query := `
		...
		DO UPDATE SET
		    ...
		    description   = COALESCE(EXCLUDED.description, reading.books.description),
		    page_count    = COALESCE(EXCLUDED.page_count, reading.books.page_count),
		    metadata_source = COALESCE(
		        EXCLUDED.metadata_source, reading.books.metadata_source
		    ),
		    updated_at    = now()
...
func (repo *BooksRepository) DeleteOrphanedBook(
	...
	query := `
		DELETE FROM reading.books
		WHERE id = $1
		  AND NOT EXISTS (
		      SELECT 1 FROM reading.user_books WHERE book_id = $1
		  )
	`
...
func (repo *BooksRepository) UpsertUserBook(
	...
	query := `
		INSERT INTO reading.user_books
		...
		ON CONFLICT (user_id, book_id) DO UPDATE SET
		    ...
		    rating          = COALESCE(EXCLUDED.rating, reading.user_books.rating),
...
func (repo *BooksRepository) BatchUpsert(
	...
	upsertUserBookQuery := `
		...
		ON CONFLICT (user_id, book_id) DO UPDATE SET
		    ...
		    rating          = COALESCE(EXCLUDED.rating, reading.user_books.rating),
		    finished_at     = EXCLUDED.finished_at,
		    added_at        = COALESCE(reading.user_books.added_at, EXCLUDED.added_at),
		    updated_at      = now()
	`
...
// It does NOT touch reading.books (the shared catalog).
func (repo *BooksRepository) DeleteUserBook(
	ctx context.Context,
	userID string,
	bookID uuid.UUID,
) error {
	query := `DELETE FROM reading.user_books WHERE user_id = $1 AND book_id = $2`
...
func (repo *BooksRepository) UpsertKoboRemoval(
	...
	query := `
		INSERT INTO reading.kobo_removals (user_id, book_id)
		VALUES ($1, $2)
...
func (repo *BooksRepository) DeleteKoboRemoval(
	...
	query := `DELETE FROM reading.kobo_removals WHERE user_id = $1 AND book_id = $2`
	_, err := repo.db.Exec(ctx, query, userID, bookID)
	return postgres.PgxErrorToHTTPError(err)
}
...
func (repo *BooksRepository) ListShelves(
	...
	query := `SELECT name FROM reading.shelves WHERE user_id = $1`
	rows, err := repo.db.Query(ctx, query, userID)
	if err != nil {
		return nil, postgres.PgxErrorToHTTPError(err)
	}
...
func (repo *BooksRepository) RenameShelf(
	...
	registryQuery := `
		WITH removed AS (
			DELETE FROM reading.shelves WHERE user_id = $1 AND name = $2
		)
		INSERT INTO reading.shelves (user_id, name)
		VALUES ($1, $3)
		ON CONFLICT DO NOTHING
...
func (repo *BooksRepository) DeleteShelf(
	...
	deleteQuery := `DELETE FROM reading.shelves WHERE user_id = $1 AND name = $2`
