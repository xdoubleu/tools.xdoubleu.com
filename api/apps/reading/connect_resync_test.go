...
func TestConnectStartResync_Force(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.StartResyncRequest{Force: true})
	req.Header().Set("Cookie", accessToken.String())
...
func TestBuildResyncProposals_Service(t *testing.T) {
	...
	_, err := testDB.Exec(ctx,
		`UPDATE reading.books SET description = NULL WHERE id = $1`, book.BookID)
	require.NoError(t, err)
...
func TestConnectCancelResync_NonAdmin_PermissionDenied(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.CancelResyncRequest{})
	req.Header().Set("Cookie", accessToken.String())
