...
import (
	...
	bsvc "tools.xdoubleu.com/apps/reading/internal/services"
...
func TestUploadFile_CanonicalStorageKey(t *testing.T) {
	...
	_, _ = testDB.Exec(context.Background(),
		`DELETE FROM reading.book_files WHERE book_id = $1`, ub.BookID)
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.book_files WHERE book_id = $1`, ub.BookID)
	})
...
func TestUploadFile_GlobalDedup_CrossUser(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.user_books WHERE user_id = $1`, userB)
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.book_files WHERE user_id = $1`, userB)
	})
...
func TestUploadFile_EPUB_ExternalSearchFallback(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.user_books WHERE user_id = $1`, isolatedUser)
	})
...
func TestConnectCreateBookUpload_OK(t *testing.T) {
	...
	req := connect.NewRequest(&readingv1.CreateBookUploadRequest{
...
func TestConnectFinalizeBookUpload_OK(t *testing.T) {
	...
	createReq := connect.NewRequest(&readingv1.CreateBookUploadRequest{
		Filename:    "handler-test.epub",
		ContentType: "application/epub+zip",
		Size:        int64(len(data)),
	})
	...
	finalReq := connect.NewRequest(&readingv1.FinalizeBookUploadRequest{
...
func noExternalMatchApp(t *testing.T, isolatedUser string) *reading.Reading {
	t.Helper()
	return reading.NewInner(
		sharedmocks.NewMockedAuthService(isolatedUser),
		testApp.Logger,
		testCfg,
		testDB,
...
func TestUploadFile_EPUB_MatchByNormalizedTitle_Subtitle(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.user_books WHERE user_id = $1`, isolatedUser)
		_, _ = testDB.Exec(context.Background(),
			`DELETE FROM reading.book_files WHERE user_id = $1`, isolatedUser)
	})
