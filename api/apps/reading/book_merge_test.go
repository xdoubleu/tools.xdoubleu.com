...
func cleanupMergeUser(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.user_books WHERE user_id = $1`, mergeTestUser)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.book_files WHERE user_id = $1`, mergeTestUser)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.book_reading_state WHERE user_id = $1`, mergeTestUser)
	})
}
...
func insertReadingState(
	...
	t.Helper()
	_, err := testDB.Exec(context.Background(), `
		INSERT INTO reading.book_reading_state (user_id, book_id, source, percent)
		VALUES ($1, $2, 'web', $3)
...
func TestMergeBooks_UnionsTagsAndFinishedAt(t *testing.T) {
	...
	_, err := testDB.Exec(context.Background(),
		`UPDATE reading.user_books SET finished_at = $1
	...
	err = testDB.QueryRow(context.Background(),
		`SELECT tags FROM reading.user_books WHERE user_id = $1 AND book_id = $2`,
	...
	err = testDB.QueryRow(
		context.Background(),
		`SELECT finished_at FROM reading.user_books WHERE user_id = $1 AND book_id = $2`,
		mergeTestUser,
		winner.BookID,
	).Scan(&finishedAt)
	require.NoError(t, err)
...
func TestMergeBooks_PicksMostProgressedStatus(t *testing.T) {
	...
	var status string
	err = testDB.QueryRow(context.Background(),
		`SELECT status FROM reading.user_books WHERE user_id = $1 AND book_id = $2`,
		mergeTestUser, winner.BookID,
	).Scan(&status)
	require.NoError(t, err)
...
func TestMergeBooks_RepointsBookFiles(t *testing.T) {
	...
	err = testDB.QueryRow(context.Background(),
		`SELECT book_id FROM reading.book_files
...
func TestMergeBooks_DeduplicatesIdenticalFiles(t *testing.T) {
	...
	var fileCount int
	err = testDB.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM reading.book_files
...
func TestMergeBooks_ConsolidatesReadingState(t *testing.T) {
	...
	var percent int
	err = testDB.QueryRow(context.Background(),
		`SELECT percent FROM reading.book_reading_state
	...
	err = testDB.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM reading.book_reading_state
		 WHERE user_id = $1 AND book_id = $2`,
		mergeTestUser, loser.BookID,
...
func TestMergeBooks_FallsBackToLoserRating(t *testing.T) {
	...
	rating := int16(4)
	_, err := testDB.Exec(context.Background(),
		`UPDATE reading.user_books SET rating = $1
		 WHERE user_id = $2 AND book_id = $3`,
	...
	err = testDB.QueryRow(
		context.Background(),
		`SELECT rating FROM reading.user_books WHERE user_id = $1 AND book_id = $2`,
		mergeTestUser,
		winner.BookID,
	).Scan(&gotRating)
	require.NoError(t, err)
...
func TestMergeBooks_AppliesResolvedMetadata(t *testing.T) {
	...
	err = testDB.QueryRow(context.Background(),
		`SELECT title, COALESCE(description, '') FROM reading.books WHERE id = $1`,
		winner.BookID,
...
func TestMergeBooks_NilResolvedMetadataPreservesBook(t *testing.T) {
	...
	err := testDB.QueryRow(context.Background(),
		`SELECT title FROM reading.books WHERE id = $1`, winner.BookID,
	).Scan(&originalTitle)
	require.NoError(t, err)
...
func TestMergeBooks_OrphanedLoserBookDeleted(t *testing.T) {
	...
	var count int
	err = testDB.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM reading.books WHERE id = $1`, loserBookID,
	).Scan(&count)
	require.NoError(t, err)
...
func TestConnectMergeBooks_InvalidCoverSourceID(t *testing.T) {
	...
	bad := "not-a-uuid"
	req := connect.NewRequest(&readingv1.MergeBooksRequest{
...
func TestConnectFindDuplicates_OK(t *testing.T) {
	client := newAdminBooksTestClient(t)
	req := connect.NewRequest(&readingv1.FindDuplicatesRequest{})
	req.Header().Set("Cookie", accessToken.String())
...
func TestConnectMergeBooks_ResolvedStatusApplied(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = testDB.Exec(
			context.Background(),
			`DELETE FROM reading.user_books WHERE user_id = $1 AND status = 'connect-shelf'`,
			userID,
		)
	})
...
func cleanupMergeUser2(t *testing.T) {
	...
	t.Cleanup(func() {
		ctx := context.Background()
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.user_books WHERE user_id = $1`, mergeTestUser2)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.book_files WHERE user_id = $1`, mergeTestUser2)
		_, _ = testDB.Exec(ctx,
			`DELETE FROM reading.book_reading_state WHERE user_id = $1`, mergeTestUser2)
	})
}
...
func TestMergeBooks_CrossUserConsolidation(t *testing.T) {
	...
	var catCount int
	err = testDB.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM reading.books WHERE id = $1`, loser.BookID,
	).Scan(&catCount)
	require.NoError(t, err)
