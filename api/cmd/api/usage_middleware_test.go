...
func TestUsageLabels(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "oauth2 register sub-path",
			method:       http.MethodPost,
			path:         "/oauth2/register",
			wantApp:      "global",
			wantEndpoint: "oauth2/register",
			wantOK:       true,
		},
		{
			name:         "oauth2 authorize sub-path",
			method:       http.MethodGet,
			path:         "/oauth2/authorize",
			wantApp:      "global",
			wantEndpoint: "oauth2/authorize",
			wantOK:       true,
		},
		{
			name:         "oauth2 token sub-path",
			method:       http.MethodPost,
			path:         "/oauth2/token",
			wantApp:      "global",
			wantEndpoint: "oauth2/token",
			wantOK:       true,
		},
		{
