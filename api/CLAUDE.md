...
- **`repositories/`** — Shared DB repositories over the `global` schema (users, contacts, the observability tables: `UsageRepository`, `StorageSnapshotsRepository`, `DBStatsRepository`, and `ProfileSharesRepository` for the public-profile share tokens)
- **`observability/`** — Cross-cutting instrumentation. `TrackedJob` decorates any `threading.Job` so every run sends a Sentry Cron Monitor check-in (`in_progress`, then `ok`/`error`, via `sentry-go`'s crons support — `RunEvery()` maps to the monitor's upsert schedule), panics are recovered, and failures also log at Error level (so they reach Sentry through the log handler too); wrap jobs at registration (see `apps/{todos,games,reading}/app.go`). `UsageRecorder` counts requests per `(day, app, endpoint)` in memory and flushes to `global.usage_daily`; the counting `usageMiddleware` sits in the `cmd/api` alice chain after `domainMiddleware`.
...
- Cross-cutting tables live in the `global` schema with migrations in `cmd/api/migrations/` (users, contacts, `profile_shares`, observability: `usage_daily`, `storage_snapshots`, and `oauth_connections` — one encrypted-token row per external provider, see `oauthconn/` above). The `observability.v1.ObservabilityService` DB-backed RPCs (`GetUsageStats`/`GetStorageStats`/`GetDatabaseStats` in `cmd/api/connect_observability.go`) read these plus live `pg_*` size queries. The same service also exposes four external-signal RPCs (`GetJobStats`/`GetGithubIssues`/`GetSentryIssues`/`GetDeployStatus`) plus a `GetHealthOverview` rollup in `cmd/api/connect_observability_external.go`, backed by `TrackedJob`'s Sentry Cron Monitors (via `sentryapi.ListMonitors`, not a table — background jobs used to be tracked in a `global.job_runs` table, replaced by Sentry Cron Monitoring) and the `internal/{github,sentryapi,digitalocean}` clients; each source degrades to an empty `configured=false` section when its OAuth connection is missing or the upstream call fails, so one broken source never fails the response. The Connect handlers and their query bodies are split: each handler is a thin `requireAdmin` + connect wrapper over an internal method (`usageStats`/`storageStats`/`databaseStats` and `jobStats`/`githubIssues`/`sentryIssues`/`deployStatus`) so the MCP tools reuse the exact same read logic.
- The same read methods are exposed to a local Claude CLI over a **read-only MCP server** (`cmd/api/mcp.go`, streamable-HTTP at `/monitoring/mcp`, `github.com/modelcontextprotocol/go-sdk`). Seven tools (`get_job_stats`/`get_usage_stats`/`get_storage_stats`/`get_database_stats`/`get_github_issues`/`get_sentry_issues`/`get_deploy_status`) wrap the internal methods above and each calls `requireAdmin`, so nothing mutating is reachable. Auth is **MCP OAuth 2.1**: the api is the resource server (`auth.RequireBearerToken` verifies a Supabase access token via `GoTrueService.ResolveToken`; `auth.ProtectedResourceMetadataHandler` serves `/.well-known/oauth-protected-resource…` pointing at Supabase as the authorization server), and the `web` `/oauth/consent` page drives the Supabase approval. Register locally with `claude mcp add --transport http tools-obs https://tools.xdoubleu.com/api/monitoring/mcp` (see root `README.md` for the one-time Supabase OAuth-server setup).
- A **second read-only MCP server** at `/apps/mcp` (`cmd/api/mcp_apps.go`) exposes each app's own read RPCs. Apps that implement `MCPToolProvider` (interface in `cmd/api/apps.go`) register `<app>_<rpc>` tools from an `apps/<app>/mcp.go` file that wraps their own Connect **read** handlers — only read RPCs are wrapped, so nothing mutating is reachable (e.g. mealplans `ListPlans` is served via `services.Plans.List` to skip its default-plan creation). The shared `internal/mcptools` package provides `RequireAppAccess` (mirrors `auth.AppAccess`: admin **or** the app in the user's app-access — this is the gate, not `requireAdmin`), the generic `AddReadTool`, and `Unwrap`/`Result`. Both MCP endpoints reuse the same OAuth 2.1 Bearer plumbing (`mcpBearerRoute`/`mcpResourceMetadataFor` in `cmd/api/mcp.go`). Register locally with `claude mcp add --transport http tools-apps https://tools.xdoubleu.com/api/apps/mcp`.
