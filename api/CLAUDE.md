...
- **`slack`** — thin Slack Incoming Webhook client (Block Kit payload). `Send(ctx, webhookURL, subject, body)` mirrors `mailer`'s `ErrNotConfigured` semantics (empty URL = no-op). The webhook URL is passed per call because it's runtime config (`global.notification_channel_config`), not an env var → [`adr-0020`](../docs/adr-0020-ui-configured-alert-delivery-channel.md).
...
- **`observability`** — `TrackedJob` (job timing/panic recovery → `global.job_runs`, plus the `job_duration_seconds` Prometheus histogram), `UsageRecorder` (per-endpoint request counts **and response bytes** → `global.usage_daily`; its `countingResponseWriter` in `cmd/api/usage_middleware.go` **must keep forwarding `Flush`/`Hijack`** or `progressws` WebSocket upgrades break), and the cross-app jobs registered on `main.go`'s own queue: issue notifier, weekly digest, and the transaction-latency snapshot (still needed — its `global.transaction_latency_daily` table backs both `WeeklyDigestJob`'s slow-transaction section and `GetSlowTransactions`' trending field). All threshold **alerting** is Grafana's now — issue #1528 retired the hand-rolled `ThresholdAlertJob`/`global.alert_states`; Grafana evaluates real Prometheus histograms (`http_request_duration_seconds` from `internal/middleware/metrics.go`, `job_duration_seconds`, `web_vitals_seconds`) and routes through a provisioned contact point ([`adr-0022`](../docs/adr-0022-prometheus-grafana-metrics.md)). The slow-transaction classification helpers survive in `internal/observability/jobs/slow_transactions.go` — the digest and the `/monitoring` trending list still use them ([`adr-0011`](../docs/adr-0011-slow-transaction-thresholds.md)). **Every notification source checks `global.notification_settings` before notifying**, and skips without writing to `notified_issues` when disabled, so re-enabling picks the item back up. **Delivery channel** is a separate axis: `notifications.Service.Enqueue` fans an alert out to email and/or Slack per the single global switch in `global.notification_channel_config` (`channel_mode`, plus a UI-set, DB-encrypted Slack webhook URL — no deploy secret), falling back to email if Slack-only delivery can't complete; `EnqueueEmail` (the two weekly digests) always emails regardless. This package never imports an `apps/*` package — `main.go`'s adapters bridge them → [`adr-0010`](../docs/adr-0010-two-weekly-digest-emails.md), [`adr-0011`](../docs/adr-0011-slow-transaction-thresholds.md), [`adr-0012`](../docs/adr-0012-ubuntu-release-check-on-vps.md), [`adr-0020`](../docs/adr-0020-ui-configured-alert-delivery-channel.md).
