...
- **`sharing`** (issue #1349) — logic shared by recipes/mealplans/shoppinglist's "share a whole owned resource with another user" features. `ValidateShareTarget` is the one rule every Share RPC must enforce (target non-empty, not the owner); recipes and shoppinglist already applied it inline, but mealplans' `PlanService.Share` didn't, letting a caller grant an empty or self-referencing plan share — all three now call the shared helper. `Repository` additionally generalizes the owner/user/can_edit access-grant SQL that recipes' `recipebook_access` and shoppinglist's `shoppinglist_access` implemented byte-for-byte identically (Share/Unshare/ListShares/GetAccess/ListAccessibleOwners), parameterized by schema/table at construction; both apps' repositories now delegate to it instead of each re-running the same five queries. mealplans shares per-plan rather than per-owner (`mealplans.plan_access` carries `plan_id`, not a whole-resource grant), so it only reuses the validation half — its access table stays its own.
