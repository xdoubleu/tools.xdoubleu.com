...
adds the 15 unprefixed admin-gated observability tools on top, sharing the
exact same internal methods the Connect handlers use — 14 are read, plus
...
- Target ≥80% coverage on changed code (`make test/cov/report`); generated files and `_mock.go` are excluded. `make test/cov/per-pkg` merges per-package Go coverage profiles via the repo-root `tools/merge_coverage.py` — its web-side sibling, `tools/diff_coverage.py`, does the equivalent lcov-based diff scoping for `web/` (`npm run test:cov:diff`).
