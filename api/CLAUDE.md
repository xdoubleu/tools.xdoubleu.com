...
- Production still has a live legacy GoTrue-populated `auth` schema (renamed to `auth_gotrue_legacy` during cutover) and a still-running `gotrue` container — `api/cmd/migrate-legacy-auth` is the one-shot script that copies existing users' bcrypt password hashes and verified TOTP factors into the new schema during the maintenance-window cutover; tearing the `gotrue` container itself out of `infra/` is a deliberate separate follow-up once that cutover is observed stable (see issue #1039).
