...
- **trains** — SNCB/NMBS timetable + CSA journey planning + GTFS-Realtime delay/cancellation/alert overlay, wired into the live journey detail page (not into `SearchJourneys` itself). Schema `trains`. `jobs.StaticImportJob` is a daily GTFS static import via `pkg/bmc`, swapped in atomically; **an import that starts writing something it previously did not must bump `services.importParserVersion` in the same change**, or a conditional GET keeps the deployed rows pinned to the old importer's output until the upstream feed happens to change → [`docs/spec-trains-gtfs-ingest.md`](../docs/spec-trains-gtfs-ingest.md). `trains.v1.TrainService.SearchJourneys` routes over an in-memory Connection Scan Algorithm index (`pkg/csa`) built from a rolling window of the ingested timetable and rebuilt by `jobs.RouterRefreshJob` → [`docs/spec-trains-journey-search.md`](../docs/spec-trains-journey-search.md). `jobs.RealtimePollJob` polls `pkg/bmc.FetchRealtime` (`rt/trip-update` every 30s, `rt/alert` on a slower cadence) into `services.RealtimeService`'s in-memory `models.Snapshot` — decoding **must** keep the GTFS-RT trip-level `CANCELED` and stop-level `SKIPPED`/`NO_DATA` `schedule_relationship` values distinct, since conflating them silently manufactures false cancellations → [`docs/spec-trains-realtime-ingest.md`](../docs/spec-trains-realtime-ingest.md). `web/app/trains` (station pickers, route overview) and `web/app/trains/[journeyId]` (the live journey page) are its user-visible pages — `trains.v1.TrainService` also serves `SearchStations` (station type-ahead), `GetFeedInfo` (drives the required CC BY attribution string), and `GetJourneyDetail` (full live leg/stop state for one journey, keyed by an opaque `journey_id` that round-trips a `SearchJourneys` result's board/alight stops and times with no server-side storage) alongside `SearchJourneys`. `services.JourneyWSService` exposes a `wstools` topic per subscribed live journey at `/trains/api/journeys/live`, lazily created by `GetJourneyDetail` and pushed a fresh rebuild after every `RealtimeService.Poll` cycle via `RealtimeService.OnUpdate` → [`docs/spec-trains-live-journey.md`](../docs/spec-trains-live-journey.md). **`trip_id` (static or realtime) is a daily-churning stopping-pattern variant — never persist it as a long-lived FK from user data or return it from `SearchJourneys`; group user-facing output by `trips.trip_short_name`.**
