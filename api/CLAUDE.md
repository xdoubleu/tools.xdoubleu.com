...
- **`family`** (issue #1349, #1403) — the single sharing concept, replacing per-app owner-centric sharing *and* the former standalone `contacts` (removed in #1403 once it gated access to nothing). `global.families`/`global.family_members` (one row per user, at most one family each — a user with no row is an implicit family-of-one, lazily materialized by `FamilyRepository.EnsureFamily` the first time it's asked for; each row also carries the member's own `display_name`, shown to the rest of the family, migration `00041`) and `global.family_invites` (pending-only — accepting or declining always deletes the row rather than marking it resolved, since at most one family per user means at most one pending invite per invitee). `family.v1.FamilyService`: `GetFamily`/`InviteToFamily`/`AcceptFamilyInvite`/`DeclineFamilyInvite`/`SetFamilyDisplayName`/`LeaveFamily` (`web/app/family`). `InviteByEmail` requires the invitee to already be a registered user (looked up via `auth.GetAllUsers`) and emails them off the request path — a send failure is logged, never fails the request. recipes/mealplans/shoppinglist key their data by `family_id` (via `repositories.FamilyRepository` passed into each `app.go`). Leaving a family cannot un-merge already-family-scoped data (issue #1349's confirmed decision) — the leaving user loses their membership row and starts over as a fresh solo family.
