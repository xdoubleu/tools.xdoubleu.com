...
- **trains** — SNCB/NMBS timetable + (later slices) CSA journey planning and realtime delay overlay. Schema `trains`. Only `jobs.StaticImportJob` exists so far — a daily GTFS static import via `pkg/bmc`, swapped in atomically. No `Routes()` yet. **`trip_id` is a daily-churning stopping-pattern variant — never persist it as a long-lived FK from user data; group user-facing output by `trips.trip_short_name`** → [`docs/spec-trains-gtfs-ingest.md`](../docs/spec-trains-gtfs-ingest.md).
