...
- **`family`** (issue #1349) — the family entity replacing per-app owner-centric sharing: `global.families`/`global.family_members` (one row per user, at most one family each — a user with no row is an implicit family-of-one, lazily materialized by `FamilyRepository.EnsureFamily` the first time it's asked for) and `global.family_invites` (a pending-only table, reusing `global.contacts`' invite/accept shape but simpler — accepting or declining always deletes the row rather than marking it resolved, since at most one family per user means at most one pending invite per invitee). This lands the family concept itself (`family.v1.FamilyService`: `GetFamily`/`InviteToFamily`/`AcceptFamilyInvite`/`DeclineFamilyInvite`/`LeaveFamily`, `web/app/family`) — **recipes/mealplans/shoppinglist still key their data by owner user ID, not family**, that re-key is deliberately out of scope here and tracked as a follow-up: investigating it surfaced that the three apps are more coupled than "one app at a time" assumes (e.g. `shoppinglist`'s `ListItemNames` joins directly into `recipes.recipes`/`mealplans.plans` by owner user ID for its catalog, so shoppinglist alone can't be re-keyed onto `family_id` without also touching the other two, or its catalog would silently stop reflecting a family member's recipes/plans). See issue #1380 for the full analysis and suggested shape of that migration. Leaving a family cannot un-merge already-family-scoped data once the re-key lands (issue #1349's confirmed decision) — the leaving user simply loses their membership row and starts over as a fresh solo family.
...
adds the 20 unprefixed admin-gated observability tools on top, sharing the
exact same internal methods the Connect handlers use — 18 are read, plus two
...
- **`make test/cov/report` post-processes the profile before it's uploaded**, via `tools/extend_signature_coverage.py`: Go opens a function's first coverage block at its body's opening brace, so the parameter lines of a signature `golines` wrapped over the 88-char limit belong to no block at all, and Codecov reports them as missed (the `) ... {` line as partial). That cost every newly added function 2-3 patch lines it could never cover, and sank `codecov/patch` to 71.42% on PR #1375 for code both `make test/cov/diff` and a local replay of CI's pipeline measured as 100% covered (issue #1376 — distinct from #1364's auto-discovery of the pre-dedup intermediates, same visible symptom). The script extends each such block back to its `func` keyword, matching what `diff_coverage_go.py` already effectively assumes.
