...
make dist    # packages into dist/gateway/: KoboGateway.app → .dmg, plus the raw binary
...
runner (`make dist RELEASE=${{ github.sha }} SENTRY_DSN=...`) and uploads the
result as an artifact; `docker.yml`'s app-image job downloads it into
`web/public/downloads/` before `docker build` runs — the root `Dockerfile`
just `COPY`s the two files in directly, it never builds gateway itself. See
root `CLAUDE.md`'s CI section for the full wiring.
