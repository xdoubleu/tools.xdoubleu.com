...
export function useRecipes() {
  ...
  return useSWR<ListRecipesResponse, Error>('/recipes', () =>
    client.listRecipes({})
  )
}
...
export function useMealPlans() {
  ...
  return useSWR<ListPlansResponse, Error>('/recipes/plans', () =>
    client.listPlans({})
  )
}
...
export function useMealPlan(id: string) {
  const client = createServiceClient(MealPlansService)
  return useSWR<GetPlanResponse, Error>(
    id ? `/recipes/plans/${id}` : null,
