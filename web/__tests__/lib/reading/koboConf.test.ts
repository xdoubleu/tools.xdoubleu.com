import { isManagedEndpoint, KOBO_DEFAULT_ENDPOINT } from '@/lib/reading/koboConf'
...
describe('isManagedEndpoint', () => {
  ...
  it('returns true when the endpoint is under this server’s Kobo sync path', () => {
    expect(isManagedEndpoint('https://myserver/reading/kobo/TOKEN', apiUrl)).toBe(true)
  })
  ...
  it('returns false for a different server’s sync path', () => {
    expect(isManagedEndpoint('https://otherserver/reading/kobo/TOKEN', apiUrl)).toBe(false)
  })
