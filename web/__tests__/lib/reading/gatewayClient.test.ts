...
describe('configureGateway', () => {
  it('POSTs the sync URL and returns serial + original endpoint', async () => {
    ...
    const res = await configureGateway('https://api.example.com/reading/kobo/TOKEN')
    ...
    expect(mockFetch).toHaveBeenCalledWith('https://127.0.0.1:41132/configure', {
      ...
      body: JSON.stringify({ syncUrl: 'https://api.example.com/reading/kobo/TOKEN' })
    })
  })
  ...
  it('includes volumePath when given', async () => {
    ...
    await configureGateway('https://api.example.com/reading/kobo/TOKEN', '/Volumes/KOBO2')
  ...
  it('surfaces the gateway error message on failure', async () => {
    ...
    await expect(configureGateway('https://api.example.com/reading/kobo/TOKEN')).rejects.toThrow(
