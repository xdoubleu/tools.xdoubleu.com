...
import { GetSharedSteamResponseSchema } from '@/lib/gen/dashboard/v1/games_pb'
...
describe('GamesDashboardPublicLibrary', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })
  ...
  it('groups games with a Favourites section first', () => {
    ...
    render(<GamesDashboardPublicLibrary token="tok-1" />)
  ...
  it('links game cards to the public game pages', () => {
    ...
    expect(links.some((l) => l.getAttribute('href') === '/dashboard/games/tok-1/2')).toBe(true)
    expect(links.every((l) => l.getAttribute('href')?.startsWith('/dashboard/games/tok-1/'))).toBe(
      true
    )
  })
