...
jest.mock('@/lib/reading/koboDevice', () => ({
...
jest.mock('@/lib/reading/gatewayClient', () => ({
  ...jest.requireActual('@/lib/reading/gatewayClient'),
...
import KoboGatewaySetup from '@/components/reading/KoboGatewaySetup'
...
const KOBO_MANAGED = {
  ...KOBO_UNMANAGED,
  currentEndpoint: 'https://api.example.com/reading/kobo/some-token'
}
...
describe('KoboGatewaySetup — fresh device', () => {
  it('registers the device and configures via the gateway', async () => {
    ...
    expect(mockConfigureGateway).toHaveBeenCalledWith(
      'https://api.example.com/reading/kobo/my-token',
      '/Volumes/KOBOeReader'
    )
