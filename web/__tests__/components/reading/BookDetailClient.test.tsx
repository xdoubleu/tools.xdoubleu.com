...
jest.mock('@/components/reading/BookSourceSync', () => {
...
jest.mock('@/components/reading/BookProgressEditor', () => {
...
describe('BookDetailClient', () => {
  ...
  it('renders breadcrumb with Books and Library links', () => {
    render(<BookDetailClient id="ub-1" />)
    const booksLink = screen.getByText('Reading').closest('a')
    expect(booksLink).toHaveAttribute('href', '/reading')
    const libraryLink = screen.getByText('Library').closest('a')
    expect(libraryLink).toHaveAttribute('href', '/reading/library')
  })
  ...
  it('carries the search query into the Library breadcrumb link', () => {
    ...
    render(<BookDetailClient id="ub-1" />)
    const libraryLink = screen.getByText('Library').closest('a')
    expect(libraryLink).toHaveAttribute('href', '/reading/library?q=dune')
  })
