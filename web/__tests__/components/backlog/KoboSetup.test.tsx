...
jest.mock('@/lib/backlog/koboConf', () => {
  const actual = jest.requireActual('@/lib/backlog/koboConf')
  return actual
})

// Mock koboDevice helpers so tests don't need a real .kobo/version file.
jest.mock('@/lib/backlog/koboDevice', () => ({
...
api_endpoint=https://api.example.com/backlog/kobo/some-token
affiliate=Kobo`
...
describe('KoboSetup — supported browser, device not configured', () => {
  ...
  it('registers device and shows success after configure', async () => {
    ...
    expect(confHandle._writable.write).toHaveBeenCalledWith(
      expect.stringContaining('api_endpoint=https://api.example.com/backlog/kobo/my-token')
    )
