...
- `app/` — one route folder per app/domain, matching the API's service boundaries (`games/`, `books/`, `recipes/`, `mealplans/`, `shoppinglist/`, `todos/`, `watchparty/`, plus `auth/`, `contacts/`, `monitoring/`, `profile/`, `settings/`, `sharing/`, `user-management/`, `oauth/consent/`).
