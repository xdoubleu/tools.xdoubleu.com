...
- `app/feeds/` — top-level RSS/newsletter reader (issue #476/#595 data lives in the reading app's `FeedService`, but this route is promoted to the home page's top level so the daily unread-reading habit is one click away instead of nested under `/reading`). `/reading/feed` redirects here. `components/reading/FeedReaderClient.tsx` is the unread-items grid; `components/reading/FeedsHeader.tsx` owns the page header row (title + the `Manage feeds` toggle) and the collapsed-by-default subscribe/manage panel it expands beneath it (`FeedManager` + `AddFeedForm`). The header lives inside `SWRFallback` because it reads `swrKeys.bookFeeds` to auto-expand when the user has no feeds yet.
