...
- `app/` — one route folder per app/domain, matching the API's service boundaries (`games/`, `books/`, `feeds/`, `recipes/`, `mealplans/`, `shoppinglist/`, `todos/`, `icsproxy/`, `watchparty/`, plus `auth/`, `contacts/`, `monitoring/`, `dashboard/`, `settings/`, `sharing/`, `user-management/`, `oauth/consent/`). `dashboard/{games,reading}/` holds both the private (owner) and public (token-shared) Games/Reading dashboards (issue #737) — `games/`/`books/` no longer have a dashboard-shaped route of their own, only library/detail/settings pages.
